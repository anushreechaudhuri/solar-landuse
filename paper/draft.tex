...
Despite this growing body of work, three critical gaps remain. First, nearly all causal evidence on solar land use impacts comes from the United States and Europe; the Global South---where land competition is most acute---remains largely unstudied at scale\citep{contextnews2024}. Second, existing analyses rely on static land cover snapshots or simple before--after comparisons, which cannot distinguish solar-induced changes from secular trends in urbanisation and agricultural intensification\citep{wuepper2025}. Third, standard satellite land cover products such as Dynamic World\citep{brown2022} do not include solar panels as a classification category---by design, since their goal is to characterise natural and anthropogenic land cover globally, not to identify specific infrastructure types. Solar-covered land is consequently labelled as bare ground, built-up area, or other existing classes, making it impossible to disentangle solar-driven land cover change from the underlying transitions using these products alone. Dedicated solar detection datasets like the Global Renewables Watch\citep{robinson2025} address this gap but do not provide the surrounding land cover context needed for impact assessment.
...
Country-level decomposition reveals marked geographic variation. In India ($n = 5{,}144$) and Pakistan ($n = 485$), solar installations predominantly replaced cropland (41\% and 35\%, respectively) and semi-arid scrubland. In contrast, Sri Lanka ($n = 114$) and Bhutan ($n = 6$) show tree/forest-dominated pre-solar land cover (35\% and 34\%), consistent with installations in more forested terrain. Bangladesh's anomalously high built-up fraction (36\%) likely reflects known limitations of Dynamic World in dense rural South Asian landscapes\citep{comparativeval2024}---where smallholder plots, homesteads, and rice paddies create spectrally ambiguous mosaics---rather than genuine prior urbanisation, a finding corroborated by our VLM validation (Section~\ref{sec:vlm}).
...
\paragraph{DW--VLM divergence in South Asian landscapes.}
Comparison between DW and VLM classifications across 30 matched images revealed a mean disagreement rate of 70\%. DW estimates substantially more forest ($+23.1\pp$) and water ($+14.3\pp$) relative to VLM, while VLM identifies more cropland ($+36.3\pp$). These divergences likely reflect the spectral ambiguity of wet-season rice paddies (classified as water by DW) and scattered tree cover in agricultural mosaics. This finding highlights the value of VLM-based validation for studies using DW for agricultural monitoring in South Asia, particularly in landscapes where smallholder agriculture, water management, and vegetation intermix at sub-pixel scales.
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    VLM models identify solar panels as a distinct class, whereas DW---which does not include solar in its class taxonomy---distributes this area across bare ground (27.3\%) and built-up (29.6\%).
  }
...
The 70\% disagreement rate between Dynamic World and VLM classifications highlights a broader concern: global land cover products may be less well-calibrated for South Asian agricultural landscapes than their aggregate accuracy statistics suggest. DW's tendency to assign higher water fractions ($+14.3\pp$) in Bangladesh likely reflects the spectral similarity between wet rice paddies and open water---a known challenge for automated classifiers in monsoon agriculture regions. This does not represent a failure of DW, which is designed for global, near-real-time land cover mapping across nine broad classes, but rather illustrates the value of supplementary approaches like VLMs that can be prompted with region-specific context. By combining DW's temporal coverage with VLM's ability to identify solar infrastructure and resolve spectrally ambiguous landscapes, we obtain a more complete picture than either approach alone.
