...
def load_sites(max_sites=None, country=None, site_ids_file=None):
    """Load treatment sites from unified DB on the volume.

    Args:
        max_sites: Limit to first N sites.
        country: Filter by country name.
        site_ids_file: Path to JSON file with list of site_id strings to include.
                       Can be a local path or a path on the Modal volume.
    """
    import json
    ...
    # Load site ID filter if provided
    allowed_ids = None
    if site_ids_file:
        import os
        # Try Modal volume path first, then local
        for path in [f"{VOL_PATH}/{site_ids_file}", site_ids_file]:
            if os.path.exists(path):
                with open(path) as f:
                    filter_data = json.load(f)
                # Support both a flat list of IDs and a dict with "site_ids" key
                if isinstance(filter_data, list):
                    allowed_ids = set(filter_data)
                elif isinstance(filter_data, dict) and "site_ids" in filter_data:
                    allowed_ids = set(filter_data["site_ids"])
                elif isinstance(filter_data, dict) and "sites" in filter_data:
                    allowed_ids = set(s["site_id"] for s in filter_data["sites"])
                else:
                    allowed_ids = set(filter_data.keys())
                print(f"Loaded {len(allowed_ids)} site IDs from {path}")
                break
        if allowed_ids is None:
            raise FileNotFoundError(f"Site IDs file not found: {site_ids_file}")
    ...
    for e in db:
        if (e["treatment_group"] == "operational"
            ...
            if country and e["country"].lower() != country.lower():
                continue
            if allowed_ids and e["site_id"] not in allowed_ids:
                continue
            polygon = None
...
# ── Stage 1b: Additional EO datasets (annual) ────────────────────────────────

@app.function(
    image=image,
    secrets=[modal.Secret.from_name("gee-credentials")],
    volumes={VOL_PATH: vol},
    timeout=600,  # 10 min — multiple GEE queries per call
    retries=modal.Retries(max_retries=2, initial_delay=10.0, backoff_coefficient=2.0),
    max_containers=8,
)
def query_eo_site_year(site: dict, year: int) -> dict:
    """Query VIIRS, SAR, LST, EVI, WorldPop, Buildings for one site × year."""
    import ee
    import json
    from pathlib import Path

    site_id = site["site_id"]
    cache_path = Path(f"{VOL_PATH}/eo_cache/{site_id}_{year}.json")
    ...
    buffer_m = compute_buffer(polygon) if polygon else 1000

    try:
        if polygon and polygon.get("coordinates"):
            geom = ee.Geometry.Polygon(polygon["coordinates"]).buffer(buffer_m)
        else:
            geom = ee.Geometry.Point([lon, lat]).buffer(buffer_m)
    except Exception:
        geom = ee.Geometry.Point([lon, lat]).buffer(buffer_m)

    result = {"site_id": site_id, "year": year}

    # 1. VIIRS nighttime lights (annual median)
    try:
        ...
        if viirs.size().getInfo() > 0:
            ...
            median_img = viirs.map(mask_quality).select("avg_rad").median()
            stats = median_img.reduceRegion(
                reducer=ee.Reducer.mean().combine(ee.Reducer.stdDev(), sharedInputs=True),
                geometry=geom, scale=463, maxPixels=1e7).getInfo()
            result["viirs_avg_rad"] = stats.get("avg_rad_mean")
            result["viirs_avg_rad_sd"] = stats.get("avg_rad_stdDev")
    except Exception:
        pass

    # 2. Sentinel-1 SAR (6-month composite Apr-Oct)
    try:
        s1 = (ee.ImageCollection("COPERNICUS/S1_GRD")
              .filterBounds(geom)
              .filterDate(f"{year}-04-01", f"{year}-10-01")
        ...
        if n_scenes > 0:
            stats = s1.median().reduceRegion(
                reducer=ee.Reducer.mean(), geometry=geom, scale=10, maxPixels=1e7).getInfo()
            result["sar_vv_db"] = stats.get("VV")
            result["sar_vh_db"] = stats.get("VH")
            result["sar_n_scenes"] = n_scenes
    except Exception:
        pass

    # 3. MODIS LST (annual mean day/night)
    try:
        lst = (ee.ImageCollection("MODIS/061/MOD11A2")
               .filterDate(f"{year}-01-01", f"{year}-12-31")
               .select(["LST_Day_1km", "LST_Night_1km"]))
        if lst.size().getInfo() > 0:
            mean_c = lst.mean().multiply(0.02).subtract(273.15)
            stats = mean_c.reduceRegion(
                reducer=ee.Reducer.mean(), geometry=geom, scale=1000, maxPixels=1e7).getInfo()
            result["lst_day_c"] = stats.get("LST_Day_1km")
            result["lst_night_c"] = stats.get("LST_Night_1km")
    except Exception:
        pass

    # 4. MODIS EVI (annual mean, QA-filtered)
    try:
        ...
        if modis_vi.size().getInfo() > 0:
            ...
            evi_img = modis_vi.map(mask_qa).select("EVI").mean().multiply(0.0001)
            stats = evi_img.reduceRegion(
                reducer=ee.Reducer.mean(), geometry=geom, scale=250, maxPixels=1e7).getInfo()
            result["evi_mean"] = stats.get("EVI")
    except Exception:
        pass

    # 5. WorldPop (annual, capped at 2020)
    try:
        query_year = min(year, 2020)
        wp = (ee.ImageCollection("WorldPop/GP/100m/pop")
              .filterDate(f"{query_year}-01-01", f"{query_year}-12-31")
              .mosaic())
        stats = wp.reduceRegion(
            reducer=ee.Reducer.sum().combine(ee.Reducer.mean(), sharedInputs=True),
            geometry=geom, scale=100, maxPixels=1e7).getInfo()
        result["pop_sum"] = stats.get("population_sum")
        result["pop_mean"] = stats.get("population_mean")
        result["pop_year_actual"] = query_year
    except Exception:
        pass

    # 6. Google Open Buildings (annual, 2016-2023)
    try:
        query_year = max(min(year, 2023), 2016)
        bldg = (ee.ImageCollection("GOOGLE/Research/open-buildings-temporal/v1")
                .filterDate(f"{query_year}-01-01", f"{query_year}-12-31")
                .filterBounds(geom))
        if bldg.size().getInfo() > 0:
            stats = bldg.mosaic().reduceRegion(
                reducer=ee.Reducer.mean(), geometry=geom, scale=4, maxPixels=5e7).getInfo()
            result["bldg_presence"] = stats.get("building_presence")
            result["bldg_height_m"] = stats.get("building_height")
            result["bldg_frac_count"] = stats.get("building_fractional_count")
            result["bldg_year_actual"] = query_year
    except Exception:
        pass
...
def collect_eo(max_sites: int = None, country: str = None):
    """Stage 1b: Collect additional EO datasets (VIIRS, SAR, LST, EVI, WorldPop, Buildings)."""
    import csv
    import time
    from pathlib import Path

    sites = load_sites(max_sites=max_sites, country=country)
    print(f"Stage 1b: EO datasets for {len(sites)} sites × {len(YEARS)} years")

    tasks = [(site, year) for site in sites for year in YEARS]
    print(f"Total tasks: {len(tasks):,}")

    n_cached = sum(1 for s, y in tasks
                   if Path(f"{VOL_PATH}/eo_cache/{s['site_id']}_{y}.json").exists())
    print(f"Already cached: {n_cached:,}/{len(tasks):,}")
    ...
    for i, result in enumerate(query_eo_site_year.map(
        [t[0] for t in tasks],
        [t[1] for t in tasks],
        order_outputs=False,
    )):
        ...
        if (i + 1) % 500 == 0:
            ...
            rate = (i + 1) / elapsed
            print(f"  [{i+1:,}/{len(tasks):,}] {rate:.1f}/sec, "
                  f"ok={i+1-errors}, errors={errors}")

    # Save CSV
    if results:
        csv_path = Path(f"{VOL_PATH}/eo_annual_panel.csv")
        all_cols = sorted(set(k for r in results for k in r.keys()))
        results.sort(key=lambda r: (r["site_id"], r.get("year", 0)))
    ...
    elapsed = time.time() - start
    print(f"Stage 1b complete: {elapsed/60:.1f} min, "
          f"ok={len(results)-errors}, errors={errors}")
...
def run_vlm(max_sites: int = None, country: str = None, site_ids_file: str = None):
...
def main(
    ...
    Args:
        ...
        stage: "all", "dw", "eo", "s2", "vlm", or "status"
        ...
        site_ids: Path to JSON file with site IDs to filter to (on volume or local).
                  Supports: list of IDs, {"site_ids": [...]}, or {"sites": [{site_id: ...}]}
    """
    print(f"Solar Land-Use Pipeline")
    print(f"  Stage: {stage}")
    print(f"  Max sites: {max_sites or 'all'}")
    print(f"  Country: {country or 'all'}")
    print(f"  Site IDs filter: {site_ids or 'none'}")
    print(f"  Detach: {detach}")

    # Upload site IDs file to volume if it's a local path
    if site_ids and not site_ids.startswith(VOL_PATH):
        import os
        if os.path.exists(site_ids):
            import subprocess
            vol_dest = f"vlm_validation_sites.json"
            result = subprocess.run(["modal", "volume", "put", "solar-landuse-data",
                          site_ids, vol_dest, "--force"],
                          capture_output=True, text=True)
            if result.returncode == 0:
                print(f"  Uploaded {site_ids} to volume as {vol_dest}")
            else:
                # --force may not be supported; file likely already exists
                print(f"  Volume file {vol_dest} already exists (skipping upload)")
            site_ids = vol_dest
    ...
    if detach:
        ...
        if stage in ("all", "eo"):
            fc = collect_eo.spawn(max_sites=max_sites, country=country)
            print(f"  Spawned EO collection: {fc.object_id}")
        if stage in ("all", "s2"):
            fc = collect_s2.spawn(max_sites=max_sites, country=country)
            print(f"  Spawned S2 collection: {fc.object_id}")
        if stage in ("all", "vlm"):
            fc = run_vlm.spawn(max_sites=max_sites, country=country, site_ids_file=site_ids)
    ...
    if stage in ("all", "eo"):
        collect_eo.remote(max_sites=max_sites, country=country)

    if stage in ("all", "s2"):
        collect_s2.remote(max_sites=max_sites, country=country)

    if stage in ("all", "vlm"):
        run_vlm.remote(max_sites=max_sites, country=country, site_ids_file=site_ids)
